...
bool UserDict::reset(const char *file) {
  ...
  size_t wred = (unsigned)fwrite(&version, 1, 4, fp);
  UserDictInfo info;
...
bool UserDict::validate(const char *file) {
  ...
  readed = (unsigned)fread(&version, 1, sizeof(version), fp);
  ...
  readed = (unsigned)fread(&dict_info, 1, sizeof(dict_info), fp);
...
void UserDict::write_back_sync(int fd) {
  int err = (int)lseek(fd, 4 + dict_info_.lemma_size
...
void UserDict::write_back_offset(int fd) {
  int err = (int)lseek(fd, 4 + dict_info_.lemma_size, SEEK_SET);
  if (err == -1)
    return;
...
void UserDict::write_back_lemma(int fd) {
  int err = (int)lseek(fd, 4, SEEK_SET);
  if (err == -1)
    return;
  ...
  err = (int)lseek(fd, dict_info_.lemma_size - need_write, SEEK_CUR);
  if (err == -1)
    return;
  ...
  for (i = 0; i < dict_info_.sync_count; i++) {
    ...
    for (j = 0; j < nchar; j++) {
      int ret_len = spl_trie->get_spelling_str16(
          spl[j], ptemp, (unsigned)(temp + sizeof(temp) - ptemp));
      if (ret_len <= 0)
        break;
      ptemp += ret_len;
    ...
    int ret_len = utf16le_lltoa(intf, ptemp, (int)(temp + sizeof(temp) - ptemp));
    if (ret_len <= 0)
      continue;
    ptemp += ret_len;
    if (ptemp < temp + sizeof(temp) - 1) {
    ...
    ret_len = utf16le_lltoa(last_mod, ptemp, (int)(temp + sizeof(temp) - ptemp));
    if (ret_len <= 0)
      continue;
    ptemp += ret_len;
    if (ptemp < temp + sizeof(temp) - 1) {
